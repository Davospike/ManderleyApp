...
class LineChart extends React.Component {
  constructor(props) {
    ...
    let newState = initData(
      ...
      this.props.gap,
      this.props.units,
      this.props.numberOfYAxisGuideLines
    );
    this.state = {
      loading: false,
  ...
  drawCoordinate(
    ...
    return (
      ...
      >
        ...
        >
          {/* don't show joinging lines if tickbox/scale */}
          {/* {this.state.trackerType === "Numeric" ? (
            <View
              style={StyleSheet.flatten([
                {
                  top: top,
  ...
  drawPoint(index, point, seriesColor) {
    ...
    if (point.empty || this.props.hidePoints) return null;
    ...
    return (
      ...
      >
        {this.state.trackerType === "Scale" ? (
          <View
            style={StyleSheet.flatten([
              styles.pointWrapper,
              {
                left:
                  point.gap - size / 2 - Dimensions.get("window").width * 0.025,
                bottom: point.ratioY - size * 1.5,
                padding: 1,
          ...
          >
            <Text style={{ color: "white", fontSize: 18 }}>
              {this.state.scaleMap[dataObject.y].emoji}
            </Text>
          </View>
        ) : this.state.trackerType === "Tickbox" ? (
          <View
            style={StyleSheet.flatten([
              styles.pointWrapper,
              {
            ...
            {this.state.sortedData.map((series) => {
              ...
              return (
                <View key={series.seriesName}>
                  ...
                  >
                    {/* colour blob next to number in tooltip */}
                    {/* <View
                      style={{
                        width: 10,
                        height: 5,
                    ...
                    {dataObject.empty !== true ? (
                      this.state.trackerType === "Numeric" ? (
                        ...
                        dataObject.data.map((tick, index) => (
                          <TickIcon key={index} tick={tick} />
                        ))
                      ) : this.state.trackerType === "Scale" ? (
                        <Text style={styles.tooltipValue}>
                          {`${this.state.scaleMap[dataObject.y].statement}`}
                        </Text>
                      ) : null
                    ) : null}
                  </View>
...
const styles = StyleSheet.create({
  ...
  chartViewWrapper: {
    ...
    alignItems: "flex-end",
    margin: 0,
    paddingRight: 80, //GIVE MORE SPACE TO THE RIGHT OF THE CHART
    overflow: "hidden",
    paddingBottom: 10,
  },
  coordinateWrapper: {
